    ...
    <header className="sticky top-0 z-50 w-full bg-white/95 backdrop-blur-md border-b border-gray-200/70 transition-all">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 sm:h-20 flex items-center justify-between">
        ...
        <a href="#" className="flex items-center group shrink-0 transition-transform duration-300 hover:scale-105">
          <span className="text-2xl sm:text-3xl font-extrabold text-black tracking-tighter">
            BFF
          </span>
        </a>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="hidden sm:inline-flex bg-black hover:bg-neutral-800 text-white text-xs sm:text-sm font-semibold px-4 sm:px-5 py-2 sm:py-2.5 rounded-sm transition-all duration-200 shadow-sm active:scale-95 cursor-pointer shrink-0"
          >
            Join the Waitlist
          </button>
          ...
          <div className="flex flex-col space-y-3 pt-2">
            {navLinks.map((link) => (
              <a
                ...
                className="text-sm font-medium text-gray-700 hover:text-black px-2 py-2 rounded-md hover:bg-gray-50 transition-colors"
              >
                {link.name}
              </a>
            ))}
            <div className="pt-2 sm:hidden">
              <button
                ...
                className="w-full bg-black hover:bg-neutral-800 text-white text-sm font-semibold py-3 rounded-sm transition-colors text-center cursor-pointer"
              >
                Join the Waitlist
              </button>
            </div>
